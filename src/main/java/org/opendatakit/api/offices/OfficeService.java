...
@Api(value = "/offices", description = "ODK Office API", authorizations = { @Authorization(value = "basicAuth") })
@Path("offices")
public class OfficeService {
	...
	public Response getList() throws IOException {
		...
		return Response.ok().entity(regionalOffices).encoding(BasicConsts.UTF8_ENCODE).type(MediaType.APPLICATION_JSON)
	...
	public Response getOffice(@PathParam("officeId") String officeId) {
		...
		try {
			OdkRegionalOfficeTable.assertRelation(callingContext);
			OdkRegionalOfficeTable record = OdkRegionalOfficeTable.getRecordFromDatabase(officeId, callingContext);
			if (record == null) {
		...
		return Response.ok().entity(office).encoding(BasicConsts.UTF8_ENCODE).type(MediaType.APPLICATION_JSON)
				.header(ApiConstants.OPEN_DATA_KIT_VERSION_HEADER, ApiConstants.OPEN_DATA_KIT_VERSION)
				.header("Access-Control-Allow-Origin", "*").header("Access-Control-Allow-Credentials", "true").build();
	}
	...
	@DELETE
	@Path("{officeId}")
	public Response deleteOffice(@PathParam("officeId") String officeId) throws IOException, DatastoreFailureException {
		Datastore ds = callingContext.getDatastore();
		User user = callingContext.getCurrentUser();
		try {
			OdkRegionalOfficeTable recordToDelete = OdkRegionalOfficeTable.getRecordFromDatabase(officeId,
					callingContext);
