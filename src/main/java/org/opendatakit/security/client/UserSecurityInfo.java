...
/**
 * Heavy object comparable to User that is transportable across the GWT interfaces. If requested, it
 * provides the full set of authorizations and group memberships that a user possesses (so that the
 * client can show or hide tabs as appropriate). The username is a unique key for this object.
 *
...
public class UserSecurityInfo implements Comparable<UserSecurityInfo>, Serializable {
  ...
  */
  ...
  /**
   * Simple converter of user's primary key to a display name. The actual user record at the time of
   * the storage is identified by the uriUser. The user's display name is encoded in that as well as
