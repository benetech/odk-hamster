...
public class OdkRegionalOfficeTable extends CommonFieldsBase {
  ...
  private static final DataField REGIONAL_OFFICE_ID =
      new DataField("REGIONAL_OFFICE_ID", DataField.DataType.STRING, true);

  private static final DataField REGIONAL_OFFICE_NAME =
      new DataField("REGIONAL_OFFICE_NAME", DataField.DataType.STRING, true);
  ...
  protected OdkRegionalOfficeTable(OdkRegionalOfficeTable ref, User user) {
    super(ref, user);
  }
  ...
  public static final OdkRegionalOfficeTable getRecordFromDatabase(String officeId,
    ...
    try {
      ...
      Query q =
          ds.createQuery(prototype, "OdkRegionalOfficeTable.getAllOffices", cc.getCurrentUser());
