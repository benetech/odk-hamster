...
public class SecurityRevisionsTable extends CommonFieldsBase {
  ...
  private static final DataField LAST_REVISION_DATE =
      new DataField("LAST_REVISION", DataField.DataType.DATETIME, true);
   ...
   */
  private static synchronized final SecurityRevisionsTable assertRelation(Datastore datastore,
   ...
   * @throws ODKDatastoreException
   */
  private static synchronized final SecurityRevisionsTable getSingletonRecord(String uri,
  ...
  public static final long getLastRoleHierarchyRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(ROLE_HIERARCHY_ROW_ID, datastore, user);
    return t.getLastRevisionDate().getTime();
  }

  public static final void setLastRoleHierarchyRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(ROLE_HIERARCHY_ROW_ID, datastore, user);
  ...
  public static final long getLastRegisteredUsersRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(REGISTERED_USERS_ROW_ID, datastore, user);
    return t.getLastRevisionDate().getTime();
  }

  public static final void setLastRegisteredUsersRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(REGISTERED_USERS_ROW_ID, datastore, user);
    t.setLastRevisionDate(new Date());
    datastore.putEntity(t, user);
  ...
  public static final long getLastSuperUserIdRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(SUPER_USER_ID_ROW_ID, datastore, user);
    return t.getLastRevisionDate().getTime();
  }

  public static final void setLastSuperUserIdRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(SUPER_USER_ID_ROW_ID, datastore, user);
    t.setLastRevisionDate(new Date());
    datastore.putEntity(t, user);
  ...
  public static final long getLastPermissionsViewRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(PERMISSIONS_VIEW_ROW_ID, datastore, user);
    return t.getLastRevisionDate().getTime();
  }

  public static final void setLastPermissionsViewRevisionDate(Datastore datastore, User user)
      throws ODKDatastoreException {
    SecurityRevisionsTable t = getSingletonRecord(PERMISSIONS_VIEW_ROW_ID, datastore, user);
    t.setLastRevisionDate(new Date());
    datastore.putEntity(t, user);
